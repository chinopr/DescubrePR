...
export async function POST(request: Request) {
  ...
  if (!rateLimit.allowed) {
    return NextResponse.json(
      { error: 'Has enviado demasiadas promociones. Intenta más tarde.' },
      {
        status: 429,
  ...
  if (!publishAccess.canPublish) {
    return NextResponse.json(
      { error: publishAccess.reason || 'No tienes permisos para publicar promociones.' },
      { status: 403 }
    );
  }
  ...
  if (error) {
    return NextResponse.json({ error: 'No pudimos guardar la promoción.' }, { status: 400 });
  }
