    ...
    return (
        <div className="p-6 md:p-8 max-w-5xl mx-auto">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-black mb-1">Mis Negocios</h1>
                    <p className="text-slate-500">{businesses.length} registrado(s)</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
                    ...
                    <p className="text-lg text-slate-500 mb-4">No tienes negocios registrados</p>
                ...
                <div className="flex flex-col gap-3">
                    {businesses.map(biz => {
                        ...
                        return (
                            <div key={biz.id} className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                                    <span className="material-symbols-outlined text-primary">storefront</span>
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <h3 className="font-bold text-lg truncate">{biz.nombre}</h3>
                                        <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${badge.cls}`}>{badge.label}</span>
                                    </div>
                                    <p className="text-sm text-slate-500">{biz.municipio} &middot; {biz.categorias.join(', ')}</p>
                                </div>
                                <Link href={`/businesses/${biz.id}`} className="text-primary text-sm font-medium hover:underline shrink-0 flex items-center gap-1">
