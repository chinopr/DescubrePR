      ...
      <main className="px-4 md:px-10 py-6 md:py-8 flex flex-1 justify-center">
        <div className="layout-content-container flex flex-col max-w-[1200px] flex-1 w-full gap-8">
          ...
          <section className="flex flex-col gap-4">
            <h2 className="text-2xl font-bold text-[color:var(--foreground)]">Categories</h2>
            <CategoryChips
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="flex-1 flex flex-col gap-6">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-[color:var(--foreground)]">Featured Places</h2>
                <Link className="text-primary font-medium hover:underline text-sm" href="/map">View all</Link>
              </div>
              {loading ? (
            ...
            <div className="w-full lg:w-80 flex flex-col gap-8">
              <div className="bg-white dark:bg-slate-800 rounded-xl p-4 md:p-5 border border-slate-100 dark:border-slate-700 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white">Upcoming</h3>
                  <Link className="text-sm text-primary font-medium hover:underline" href="/events">All</Link>
                </div>
                <div className="flex flex-col gap-4">
                  {loading ? (
                    [1, 2].map(i => <div key={i} className="h-16 bg-slate-100 dark:bg-slate-700 rounded-lg animate-pulse" />)
                  ) : events.length === 0 ? (
                    <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-4">No hay eventos próximos.</p>
                  ) : (
                    events.map(event => (
                      <EventCard
                        key={event.id}
              ...
              <div className="bg-gradient-to-br from-primary/20 to-primary/5 dark:from-primary/30 dark:to-primary/10 rounded-xl p-4 md:p-5 border border-primary/20">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white">Trending Promos</h3>
                </div>
                <div className="flex flex-col gap-4">
                  {loading ? (
                    <div className="h-24 bg-primary/10 rounded-lg animate-pulse" />
                  ) : promos.length === 0 ? (
                    <p className="text-sm text-slate-500 dark:text-slate-400 text-center py-4">No hay promociones activas.</p>
                  ) : (
                    promos.map(promo => (
                      <PromoCard
                        key={promo.id}
